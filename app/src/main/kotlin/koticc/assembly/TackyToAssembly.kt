...
class TackyAssemblyGenerator(private val symbolTable: BackendSymbolTable) {
    ...
    fun tackyProgramToAssembly(tackyProgram: Tacky.Program): Assembly.Program = Assembly.Program(
        topLevel = tackyProgram.topLevel.map { tackyTopLevelToAssembly(it) } + constantMap.values.map { Assembly.TopLevel.StaticConstant(it) },
    )

    private fun tackyTopLevelToAssembly(tackyTopLevel: Tacky.TopLevel): Assembly.TopLevel = when (tackyTopLevel) {
    ...
    private fun copyParameters(parameters: List<String>): List<Assembly.Instruction> {
        ...
        val moveStackArguments = classifiedParameters.stackArguments.mapIndexed { index, parameter ->
            Assembly.Instruction.Mov(
                ...
                src = Assembly.Operand.Memory(Assembly.RegisterValue.Bp, 16 + index * 8),
                dst = Assembly.Operand.PseudoIdentifier(parameter),
            )
        }
    ...
    private fun tackyInstructionToAssembly(tackyInstruction: Tacky.Instruction): List<Assembly.Instruction> = when (tackyInstruction) {
        ...
        is Tacky.Instruction.GetAddress -> {
            listOf(
        ...
        is Tacky.Instruction.Load -> {
            listOf(
                Assembly.Instruction.Mov(
                    type = Assembly.Type.QuadWord,
                    src = tackyValueToOperand(tackyInstruction.srcPtr),
                    dst = Assembly.Operand.Register(Assembly.RegisterValue.Ax),
                ),
                Assembly.Instruction.Mov(
                    type = tackyInstruction.dst.assemblyType(),
                    src = Assembly.Operand.Memory(Assembly.RegisterValue.Ax, 0),
                    dst = tackyValueToOperand(tackyInstruction.dst),
                ),
            )
        }
        is Tacky.Instruction.Store -> {
            listOf(
                Assembly.Instruction.Mov(
                    type = Assembly.Type.QuadWord,
                    src = tackyValueToOperand(tackyInstruction.dstPtr),
                    dst = Assembly.Operand.Register(Assembly.RegisterValue.Ax),
                ),
                Assembly.Instruction.Mov(
                    type = tackyInstruction.src.assemblyType(),
                    src = tackyValueToOperand(tackyInstruction.src),
                    dst = Assembly.Operand.Memory(Assembly.RegisterValue.Ax, 0),
                ),
            )
        }
    }
    ...
    private fun functionCall(call: Tacky.Instruction.Call): List<Assembly.Instruction> = buildList {
        ...
        classifiedParameters.stackArguments.reversed().forEach { arg ->
            ...
            when {
                assemblyArgument is Assembly.Operand.Immediate ||
                    assemblyArgument is Assembly.Operand.Register ||
                    arg.assemblyType() == Assembly.Type.QuadWord ||
                    arg.assemblyType() == Assembly.Type.Double -> {
...
fun Assembly.Operand.isMemory() = when (this) {
    is Assembly.Operand.Memory -> true
